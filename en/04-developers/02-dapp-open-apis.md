...
We offer a contract product pricing feature for Sofa.org, which involves requesting quotes from MarketMakers and then integrating the best pricing results to return to Sofa.org. Subsequently, users can choose whether to trade. Due to the need to interface with different MarketMakers based on the contract, the products that can be launched need to be distinguished by contract type.

## Recommended DNT RFQ Inquiry

- Description
  - Only supports a list of RFQs composed of the same vault+chainId.
...
- Description
  - When only making an inquiry, do not pass the user's wallet address.
  - When subscribing, pass the user's wallet address.
